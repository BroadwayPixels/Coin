...
const char * const BITCOIN_CONF_FILENAME = "broadway.conf";
const char * const BITCOIN_PID_FILENAME = "broadwayd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "broadway" is a composite category enabling all Broadway-related debug output
                if(ptrCategory->count(std::string("broadway"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Broadway
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Broadway
    // Mac: ~/Library/Application Support/Broadway
    // Unix: ~/.broadway
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Broadway";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Broadway";
#else
    // Unix
    return pathRet / ".broadway";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty broadway.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override broadway.conf
