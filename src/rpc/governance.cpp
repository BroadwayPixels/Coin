...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in broadway.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("broadway.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Broadway features */
    { "broadway",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "broadway",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "broadway",               "gobject",                &gobject,                true,  {} },
    { "broadway",               "voteraw",                &voteraw,                true,  {} },
