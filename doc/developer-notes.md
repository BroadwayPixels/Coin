...
Broadway Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 10167 for connections and services them.
...
However, in open source software such as Broadway Core, where everyone uses
...
pay attention to for reviewers of Broadway Core code.

General Broadway Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Broadway Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Broadway Core so that they can be integrated
